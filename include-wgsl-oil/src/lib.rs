...
pub fn include_wgsl_oil_crated(path: proc_macro::TokenStream, module: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    // Inject items
    module.content.as_mut().expect("set to some at start").1.push(syn::parse_quote!(
        use naga;
    ));
    ...
    if let Some((_, ref mut items)) = &mut module.content {
        for item in items.iter_mut() {
            if let syn::Item::Mod(item_mod) = item {
                if item_mod.ident == "types" {
                    if let Some((_, ref mut items)) = &mut item_mod.content {
                        ...
                        for item in items {
                            if let syn::Item::Struct(r#struct) = item {
                                r#struct.attrs.push(syn::parse_quote!(
                                    #[derive(Copy)]
                                ));
                            }
                        }
...
#[proc_macro_attribute]
pub fn include_wgsl_oil_ember(path: proc_macro::TokenStream, module: proc_macro::TokenStream) -> proc_macro::TokenStream {
    // Parse module definitions and error if it contains anything
    let mut module = syn::parse_macro_input!(module as syn::ItemMod);
    if let Some(content) = &mut module.content {
        if content.1.len() > 0 {
